...
* **Contents:**
  ...
  * **Export:** `TranscriptExporter` implements `ITranscriptExporter`. The primary overload `Render(result, format, speakerNames)` accepts an `IReadOnlyDictionary<string, string>` that maps auto-generated speaker IDs to user-supplied labels; a convenience overload `Render(result, format)` delegates with an empty dict. Plain Text emits speaker headers (`DisplayName  MM:SS – MM:SS`) with paragraph text when diarized, otherwise the raw transcript string; Markdown emits a `# Transcript` heading, a duration/language metadata line, then either `## DisplayName` sections with italic timestamp ranges (diarized), timed-segment paragraphs, or flat text.
...
* **Contents:**
  ...
  * **ViewModels:** MVVM view models (using the MVVM Community Toolkit) that hold UI state, expose commands, and delegate work to injected services. Views bind to ViewModels; no logic lives in code-behind. `SpeakerNameEntry` is a lightweight `ObservableObject` (one per unique speaker) holding a read-only `SpeakerId` and an editable `CustomName`; its computed `DisplayName` falls back to `SpeakerId` when blank. `SegmentDisplayItem` subscribes to its `SpeakerNameEntry.PropertyChanged` so the transcript preview header updates live as the user types — no collection rebuild needed.
...
8. **Display (UI):** The ViewModel updates its observable properties; Avalonia bindings automatically refresh the View. When `Segments` is present, each turn is rendered as a `SegmentDisplayItem` — a bold header with speaker label and timestamps, followed by the merged transcript text. The flat `TextBox` is hidden and replaced by a `ScrollViewer`/`ItemsControl`. A "Speaker Labels" panel appears above the transcript, showing one editable row per unique speaker; typing a custom name updates the transcript header live via `PropertyChanged` propagation without rebuilding the collection. The temp WAV file is deleted in the `finally` block.
9. **Export (UI -> Infrastructure):** The user selects an `ExportFormat` (`PlainText` / `Markdown`) from a dropdown (persisted via `ISettingsService`) and optionally assigns custom names in the Speaker Labels panel. On "Save", the ViewModel builds a `Dictionary<string, string>` from non-blank `SpeakerNameEntry` values and calls `ITranscriptExporter.Render(_lastResult, SelectedExportFormat, nameMap)`. Speakers left unnamed fall back to their auto-generated IDs. The rendered string is passed — along with the appropriate file extension (`.txt` or `.md`) — to `IFileSaverService.SaveTranscriptAsync`, which opens a native Save File dialog.